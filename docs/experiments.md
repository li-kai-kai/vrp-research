...
| 工作 | 当前完成情况 | 证据与限制 |
|---|---|---|
| 回归测试 | 基线 21/21 通过；v2 当前 **130/130** 通过 | 主线与 legacy 测试；覆盖实现约束，不证明模型现实有效性 |
...
- 区分三个层次：**机制暴露（必要）→ 配送参与 → 目标效应**。本轮 80 个固定决策中，
  `*_allocations_changed` 与目标是否变化**逐行一致**（PR 30/30、HT 19/19、EC 0/0），
  因此作为后续场景筛选的**经验触发判据**；**尚未证明为一般必要充分条件**。
  只看可达性或候选路径会漏判——它们只是必要条件。
- EC 的绑定边界（**限定在本轮 S025、两类诊断拓扑、640 个网格单元内**）：
  利用率 < 0.85 从不绑定（0/517），0.85–0.97 过渡（49%），≥ 0.97 全部绑定（88/88）。
  这是经验边界，不是通用阈值；S050/M100 复核前不得当作硬阈值。
- 瓶颈判定改用**逐资源放宽对照**：车队在两类场景中都绑定；随机损伤场景中车队与供给**同时**参与，
  不是单一瓶颈；道路容量在 0.002 标度下两类场景都不绑定。
  **此前的"总供给受限"结论已撤回**——方向错误。
...
5. **EC 边界需在 S050/M100 复核**：0.85 / 0.97 目前只有 S025 证据。

...
| 工作 | 当前完成情况 | 证据与限制 |
|---|---|---|
| 回归测试 | 基线 21/21 通过；v2 当前 **193/193** 通过 | 主线与 legacy 测试；覆盖实现约束，不证明模型现实有效性 |
...
- 效应集中在**成本**：关闭 HT 使 F2 平均 −6.9%、最多 −23.3%，而送达量与 F1 几乎不变。
  符号由"HT off 是严格放宽"解析决定，经验部分是幅度。
- **60 个半真实 overlay 无一 inactive**（58 个 strong-active、2 个 active），
  损伤均匀性经测量（抽中桥比例 0.153 对基线 0.157）。
- 小预算 Full vs No-HT 搜索：Full 自回放 92 个决策误差 0；No-HT 的 96 个决策回放后 74 个目标改变。

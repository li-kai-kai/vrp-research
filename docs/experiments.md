...
- 模型价值诊断显示，在当前 S025、`capacity_scale=0.05` 的小预算诊断中，**HT/EC 未表现出稳定、系统性的目标影响**；
  ...
  道路容量利用率较低、车队趟次有限，**提示**可能存在资源瓶颈顺序，但尚未通过资源放宽对照确认。
...
1. **机制适用条件 / binding boundary 诊断（第一优先）**。回答"在什么物理和资源条件下 PR / HT / EC 会真正改变
   可行路线、配送行为或目标"，并同时保留默认不绑定区、过渡区与明确绑定区，三者都报告。
   需要先做低成本暴露度扫描（部分恢复状态的边—周期数、穿过这些边的供需路径数、边容量利用率分布），
   确认机制在哪些参数区域物理上会触发，再决定是否值得在其上做统计。
2. **固定模型价值诊断方案（第二优先）**。在已定位的绑定区内，用统一 Full 执行回放测量忽略各机制的决策损失；
   诊断必须给出全方案影响分布，而不是只看均值或固定方案。
3. **再决定正式 P6**。只有上述诊断稳定后才锁定正式实例与参数。
4. 动态实验在偏差 0/0.15/0.30、多个种子和 12 个资源设置上重跑，保留共同随机数；另外设计关键断联网络，检验适用边界。
5. 正式算法对照增加评价预算和重复。汶川固定网络必须单独使用 `--cases WEN38 --instance-seeds 1`；合成网络使用多个实例种子，不能把同一汶川网络重复命名当作独立实例。
6. 正式三因素消融按“案例 × 实例种子”分片，每片保留配对的 30 次 solver 重复；使用统一分析入口重算 pooled HV/IGD，再报告效应、区间和校正后的检验结果。
7. 保存完整运行环境、种子、预算和产物指纹后更新本页；不以新图或 smoke 完成替代正式研究完成。

...
当前全量测试：`uv run python -m unittest discover -s tests -v` → **202 个，202 通过，0 失败，0 错误**。
...
该轮完成时回归用例为 110 个（当时值）；当前总数为 202 个（含机制诊断 37 例、WEN38 天然 corridor 35 例）。
...
第二轮结束时全量测试为 130 个全通过；当前为 202 个（新增机制诊断 37 例、WEN38 天然 corridor 35 例）。第二轮的两项修复

...
从模型方法看，道路修复问题通常以受损路段、维修队、修复时间和路网连通性为核心变量，采用混合整数规划、弧路由模型或调度-路径一体化模型确定修复顺序。该类方法能够较好刻画维修队移动路径和修复任务排序，但部分研究主要评价节点恢复时间或连通性改善，对救援物资配送过程中的车辆路径、需求满足和服务公平考虑相对有限。

### 3.3 道路状态与恢复能力刻画

道路状态刻画是道路修复模型能否服务配送决策的基础。既有研究多以道路是否修复、节点是否可达或路段容量是否变化来表示路网恢复过程，这种处理有助于降低模型复杂度，但对临时抢通、限速通行、单向放行和分阶段恢复等状态表达不足。后续研究若要进一步支撑车辆路径优化，需要将道路连通性、通行时间、通行容量和车型可达性等因素纳入统一的路网状态描述。

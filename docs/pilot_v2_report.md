...
> 本轮代表方案在 18/18 运行中再次改变（拥挤距离参与环境选择，搜索过程本身变了），
> 但**质量水平基本未变**：代表方案 F1 均值 3.7933 → **3.7952**，
> 绝对变化约 **+0.0019**，相对约 **+0.05%**。
> 这里只描述数值变化，不使用"改善/恶化"——两轮的搜索规则不同，方向不构成优劣。
> 距离坐标修正改变的是"选哪一个"和"在什么坐标上量"，不是"能选到多好"。
>
...
| 唯一决策数（`runs.csv` 的 `unique_decisions`） | 逐算法均值 477 / 460 / 470（nsga2 / nsga2_ls / nsga2_alns），均低于 500；差额是同一决策被重复评价 |
...
| 算法 | 运行 | HV 均值 | HV 标准差 | IGD 均值 | 非支配点数均值 | 非支配点数中位数 | 预算内评价数均值 | 运行时长均值/s |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| `nsga2` | 6 | 1.0577 | 0.1595 | 0.1509 | 5.33 | 4.5 | 500 | 9.61 |
| `nsga2_ls` | 6 | 0.8179 | 0.1315 | 0.2850 | 5.17 | 3.5 | 500 | 9.43 |
| `nsga2_alns` | 6 | 0.9037 | 0.1999 | 0.2068 | 3.83 | 4.5 | 500 | 9.48 |

上表全部数值取自 `outputs/claude_v2_reviewfix2/pilot_algorithm/aggregate_by_size.csv`
（`*_mean` / `*_sd` / `*_median` 列），未经手工改写。

HV/IGD 使用每个实例 pooled 参考前沿与参考点 `(1.1, 1.1, 1.1)`；区间仍重叠，且 n=6，
**不能据此声称混合算法更优或更差**。非支配点数均值 5.33 / 5.17 / 3.83（中位数 4.5 / 3.5 / 4.5），
约降为首轮的 1/5（32.2→5.33、23.3→5.17、22.3→3.83），是量化去重合并亚分辨率点的直接结果，不是搜索退化；`pareto_size_sd` 分别为
1.97 / 3.82 / 2.32，说明运行间波动本身就大。

**算子选择频率**（占各组局部搜索评价的比例；这些是算子被调用的次数，不是对解的改善贡献）：
...
**这些是算子被调用的频率，不是算子对解的改善贡献。** 只有在做 remove-one-operator ablation
（逐一移除算子并比较质量变化）之后才能谈贡献；本轮没有做，因此不能推断任何算子更有用。
频率差异只说明自适应权重确实改变了选择行为。
...
|---|---|---:|---:|---:|---|---:|
| S025 i101 | `v2_service_resolution` | 1.9408e-01 | 5.2359e+02 | **4.3121e-13** | 19407657 / 523585809 / **0** | **2** |
| S025 i102 | `v2_service_resolution` | 4.1707e-01 | 6.8504e+02 | **5.4279e-13** | 41707370 / 685041515 / **0** | **2** |

**F3 的原始浮点值仍存在约 `1e-13` 量级的尾差，但在预先规定的 `1e-8` 服务分辨率下被量化到同一个比较键**，
因此 **F3 的 key span 为 0**。本轮把 F3 作为**无有效变化的常数维度**处理：
它不参与拥挤距离，也不计入 HV/IGD 的有效维度（有效维度为 2）。这是"在服务分辨率下没有变化"，
不是"原始数值完全相同"。
...
> **后续进展（2026-09-20，不改写本节历史记录）**：本节第 7 条（机制绑定边界未定位）与
> 第 8 条（"车队是瓶颈"待验证）已由[机制适用条件诊断报告](mechanism_applicability_report.md)回答。
> 该诊断**推翻了此处"车队趟次预算比道路吞吐更紧"的推测方向**：加入逐资源放宽对照后，
> 车队在两类场景中都绑定，随机损伤场景中车队与供给同时参与。
> 机制边界结论限定在 S025 与两类诊断拓扑内，不是通用阈值。
...
- 不得把"车队趟次预算比道路吞吐更紧"写成已证明的结论——本轮未做资源放宽对照。
  （后续的机制诊断已补做该对照，并**否定了这一方向**，见[机制适用条件诊断报告](mechanism_applicability_report.md)。）
...
| 存储代表方案 F1 均值 | 3.7933 | **3.7952**（绝对 +0.0019，相对 +0.05%） |
| 前沿点数合计 | 81 | 86 |
...
搜索过程本身变了。但代表方案的质量水平基本未变（F1 均值绝对变化 +0.0019，相对 +0.05%），
说明这次修正改变的是**选择与度量口径**，不是搜索能力。
方向本身不构成优劣：两轮搜索规则不同，这里只描述数值变化，不使用"改善/恶化"。
不得把它写成算法创新或性能提升。

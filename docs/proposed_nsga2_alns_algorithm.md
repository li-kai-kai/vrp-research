...
上述流程既可在期初生成 open-loop 全期方案，也可嵌入滚动时域。滚动版本不是预先知道未来道路状态：每个决策期只使用截至上一期已经观测到的修复进度、容量阶段、累计配送和剩余资源，并以 `E[xi]=1` 预测尚未实现的维修效率，然后重新优化剩余规划期。
...
当前原型不求解普通交通均衡，而是维护路段—周期残余吞吐量。车型 `v` 执行 `n` 趟配送时，从路径中每条路段扣减 `n * pcu_per_vehicle_v`。如果当前最短路容量不足，则在过滤容量不足边后的残余网络中重新计算最短路。这样，车型阈值负责判断“能不能走”，周期吞吐约束负责判断“还能走多少辆”。

### 4.4 实际维修效率与滚动时域

道路 `a` 在周期 `t` 的实际进度按下式更新：

```text
p_a,t = min(1, p_a,t-1 + xi_a,t * w_a,t / R_a)
```

其中 `w_a,t` 是本期安排的维修工时，`R_a` 是基准修复时间，`xi_a,t` 是本期决策之后才实现的维修效率。决策顺序为：

```text
读取截至 t-1 的实际状态
    -> 优化并执行 t 期维修决策
    -> xi_a,t 实现
    -> 观测 p_a,t、容量阶段和车型可通行性
    -> 执行配送
    -> 以实际状态启动 t+1 期重规划
```

期初 open-loop 基线只运行一次剩余期解码，并以 `E[xi]=1` 形成完整方案，执行中不改变维修安排。rolling 版本在每期以实际观测状态重新运行 NSGA-II + ALNS；可把上一期剩余 Pareto 解移位后作为 warm start，以控制计算时间。所有动作必须满足非预见性：周期 `t` 的维修选择不得使用尚未实现的 `xi_a,t` 或未来维修效率。
...
| max_capacity_gain_insert | 优先插入能提高瓶颈路段周期吞吐上限的道路 |
| high_demand_access_insert | 优先插入通往高需求节点的道路 |
| crew_earliest_available_insert | 将任务分配给最早可用的维修队，以减少等待并平衡作业负载 |
...
| capacity_aware_insert | 根据车型阈值和路段剩余 pcu 选择可行路径；容量不足时搜索绕行路径 |
...
| 维修效率逐期揭示 | 以已观测进度为状态，滚动重解剩余规划期并满足非预见性 |
...
| 运行时间 | 应急场景可用性 |
| 可行解比例 | 约束处理效果 |
| 最大路段利用率 | 检查吞吐约束是否实际进入解码过程 |
| 高利用率边—周期数 | 统计利用率不低于 80% 的瓶颈状态 |
| 容量阻塞配送量 | 统计因残余路段容量不足无法执行的目标配送量 |
| 车辆趟数 | 连接配送吨位、车队规模和路段 pcu 占用 |
| 修复进度预测误差 | 描述期望计划与实际道路状态的偏离程度 |
| rolling 配对收益 | 同一维修效率实现下 open-loop 与 rolling 的目标差 |
| rolling regret | 与完全信息方案或可计算下界的差；作为后续扩展 |
...
| 实验 | 目的 |
|---|---|
| `delta=0`：open-loop vs rolling | 安慰剂检查；没有新道路信息时不应出现系统差异 |
| `delta>0`：open-loop vs rolling | 识别已观测道路状态的重规划价值 |
| 完全信息方案 vs rolling | 估计 rolling regret；待实现 |
...
5. 第一版采用“速度恢复系数 + 路段周期吞吐上限”，不求解普通交通 BPR；后续取得 OD 数据后再升级。

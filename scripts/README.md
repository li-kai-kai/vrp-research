...
| `reproduce/mechanism_applicability.py` | 固定决策的 PR/HT/EC 暴露度、资源网格、维修时长扫描 | 机制适用条件诊断；不运行优化器 |
| `reproduce/mechanism_zone_search.py` | 在不绑定/过渡/绑定三个分区上做小预算搜索与统一 Full 回放 | 机制诊断阶段 2/3 |

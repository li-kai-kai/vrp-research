...
| `reproduce/ht_natural_corridor.py` | WEN38 结构扫描（桥/介数/OD 依赖/绕行比）、逐周期 threshold exposure、固定决策 HT on/off、半真实 overlay（SR-A/B/C）、小预算 Full vs No-HT 回放 | 天然 corridor 诊断；只读复用模型与共享评价器，不改 WEN38 |
